...
/**
 * Log a notice creation activity
 */
async function logNoticeCreated(noticeData, userId, userName) {
  return logActivity({
    type: 'notice_created',
    message: `Notice "${noticeData.notice_stage || 'Notice'}" created for case by ${userName || 'Unknown User'}`,
    status: 'info',
    userId: userId,
    relatedId: noticeData.case_id,
    relatedType: 'case',
    metadata: {
      notice_id: noticeData.id,
      notice_stage: noticeData.notice_stage,
...
/**
 * Log a notice update activity
 */
async function logNoticeUpdated(noticeData, userId, userName) {
  return logActivity({
    type: 'notice_updated',
    message: `Notice "${noticeData.notice_stage || 'Notice'}" updated by ${userName || 'Unknown User'}`,
    status: 'info',
    userId: userId,
    relatedId: noticeData.case_id,
    relatedType: 'case',
...
/**
 * Log a notice deletion activity
 */
async function logNoticeDeleted(noticeData, userId, userName) {
  return logActivity({
    type: 'notice_deleted',
    message: `Notice "${noticeData.notice_stage || 'Notice'}" deleted by ${userName || 'Unknown User'}`,
    status: 'warning',
    userId: userId,
    relatedId: noticeData.case_id,
    relatedType: 'case',
