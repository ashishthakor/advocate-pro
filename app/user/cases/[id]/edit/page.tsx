...
export default function UserEditCasePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const validateAndAddFiles = (newFiles: File[]) => {
    ...
    for (const file of newFiles) {
      ...
      if (!allowedTypes.includes(file.type)) {
        nextError = nextError || `${file.name} is not supported`;
        continue;
      }
      validFiles.push(file);
    }
    if (files.length + validFiles.length > MAX_FILES) nextError = nextError || `Max ${MAX_FILES} files`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.success) {
        setSuccessOpen(true);
        setTimeout(() => router.push('/user/cases'), 600);
      } else {
